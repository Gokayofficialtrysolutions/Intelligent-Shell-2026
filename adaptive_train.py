...
# Load central config to get default paths, but allow CLI to override.
try:
    import toml
    CONFIG_FILE_PATH = Path(__file__).resolve().parent / ".agi_terminal_cache" / "config.toml"
    if CONFIG_FILE_PATH.exists():
        config_data = toml.load(CONFIG_FILE_PATH)
        DEFAULT_MODEL_PATH = config_data.get("model", {}).get("merged_model_path", "./merged_model")
        DEFAULT_OUTPUT_DIR = config_data.get("model", {}).get("adapters_path", "./merged_model_adapters")
    else:
