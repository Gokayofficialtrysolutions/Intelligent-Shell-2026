# interactive_agi.py
"""
AGI Terminal - Interactive Mode

This script provides an interactive command-line interface to a merged Large Language Model (LLM),
referred to as the AGI (Artificial General Intelligence) or MergedAGI.

Core Functionalities:
- Loads a pre-merged LLM (typically from ./merged_model directory).
- Provides a chat interface for users to interact with the AGI.
- Includes a set of built-in slash commands for system interaction, file operations,
  Git operations, configuration management, and more.
- Implements a tool usage framework where the AGI can request to:
    - Execute whitelisted shell commands.
    - Read file contents.
    - Write or suggest changes to files.
    - Perform web searches.
    - Execute Python code snippets in a restricted environment.
    - Perform Git operations (branch, checkout, commit, push).
    - Execute multi-step plans composed of these tool actions.
- Maintains conversation history and logs interactions.
- Uses the 'rich' library for enhanced terminal output if available.
- Supports configuration via a TOML file (./.agi_terminal_cache/config.toml).

The AGI's responses are generated by the loaded LLM, and its ability to use tools
is guided by specific prompt engineering techniques. The system prioritizes safety
by requiring user confirmation for sensitive operations and restricting tool capabilities.
"""
...
class ContextAnalyzer:
    """
    Gathers and formats contextual information about the current environment,
    such as current working directory, Git status, file types, and snippets
    from key files. This context is provided to the AGI to help it understand
    the user's environment and generate more relevant responses.
    """
    def __init__(self):
...
class MergedAGI:
    """
    Interface to the merged Large Language Model (LLM).
    Handles model and tokenizer loading, prompt construction, text generation,
    and parameter management for the AGI.
    If the actual model fails to load, it can operate in a mock mode using AGIPPlaceholder.
    """
    def __init__(self): # model_path_str argument removed
        """
        Initializes the MergedAGI.
        Attempts to load the tokenizer and model from the path specified in the
        application configuration (APP_CONFIG['model']['merged_model_path'])
        or defaults to './merged_model'.
        Sets up device (CUDA if available, else CPU) and generation parameters.
        """
    ...
    def generate_response(self, prompt: str) -> str:
        ...
        try:
            ...
            # This is the primary output from the AGI if no tools are involved,
            # or the AGI's final textual response after tool use.
            return response_text.strip()

        except Exception as e:
            console.print(f"ERROR: Exception during model generation: {e}", style="error")
            # Ensure error_message is defined or provide a default
            error_message_default = "[bold red]Error:[/bold red] An unexpected error occurred during AGI response generation."
            return error_message_default

    def set_parameter(self, param_name_str: str, param_value_str: str) -> str:
        """
        Sets a generation parameter for the AGI model.
        Validates parameter name and value type.
        """
    ...
    def show_parameters(self) -> Union[str, Text]:
        """Displays the current generation parameters used by the AGI."""
        if not self.is_model_loaded:
            return "[yellow]Model not loaded.[/yellow] Parameters are at default values but not actively used by a real model."

        output_text = Text("Current AGI Generation Parameters:\n", style="bold")
        # Iterate through PARAM_MAP to display parameters in a consistent order with user-friendly names
        for user_friendly_name, internal_name in PARAM_MAP.items():
            value = self.generation_params.get(internal_name)
            # Format value nicely, especially for floats or None
            value_str = f"{value:.2f}" if isinstance(value, float) else str(value)
            output_text.append(f"  {user_friendly_name}: {value_str}\n")

        # Display other relevant generation parameters not directly settable via /set but important
        output_text.append(f"  DO_SAMPLE: {self.generation_params.get('do_sample')}\n") # Controls sampling vs greedy
        output_text.append(f"  PAD_TOKEN_ID: {self.generation_params.get('pad_token_id')}\n")
        output_text.append(f"  Model Max Sequence Length (from config): {self.model_max_length}\n")
        return output_text


class AGIPPlaceholder:
    """
    A placeholder for the MergedAGI when the actual model cannot be loaded.
    Provides mock responses to user inputs.
    """
    def __init__(self, model_path_str="./merged_model"): # model_path_str is for consistency
        """Initializes the placeholder AGI."""
        self.model_path = Path(model_path_str) # Store for potential reference in messages
        # No actual model loading here
        console.print("INFO: AGIPPlaceholder initialized (AGI will provide mock responses).", style="info")

    def generate_response(self, prompt: str) -> str:
        """Generates a mock response based on the user's prompt."""
